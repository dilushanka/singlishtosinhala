...
function enable_esena() {}

function notification_window() {
	var subs_topic = localStorage.getItem('sub_topic');
	if (get_alredy_subscribe_for_topic(subs_topic)) {
		var disabled_topics = enable_pop_up();
		if (!disabled_topics.includes(subs_topic)) {
			$('#enable_notifications_home').modal('show');
		}
	}
}

function get_alredy_subscribe_for_topic(topic) {
	var subscribed_topics = JSON.parse(localStorage.getItem('topics')) || [];
	var result = true;
	for (var z = 0; z < subscribed_topics.length; z++) {
		if (subscribed_topics[z] == topic) {
			result = false;
			break;
		}
	}
